...
resource "null_resource" "cleanup_old_images" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      echo "  --repository-name ${each.value} --sort-by TIMECREATED --sort-order DESC \\"
      echo "  | jq '.data.items[20:][].id' \\"
      echo "  | xargs -I{} oci artifacts container image delete --image-id {} --force"
    EOT
  }
