...
# ParticipantLobby.jsx actually handles multiple states:
# 1. if (results) -> shows results
# 2. if (questionPayload) -> shows active question
# 3. default return -> shows the waiting lobby

# We only want to replace the default return (the waiting lobby) to match the UI.
...
# Carefully replace ONLY the final return statement.
match = re.search(r'  return \(\n    <Layout>\n      <div className="mx-auto max-w-4xl.*?    </Layout>\n  \);\n}', content, re.DOTALL)
if match:
    content = content[:match.start()] + new_return
    with open('frontend_gnosis-main/frontend/src/pages/ParticipantLobby.jsx', 'w') as f:
        f.write(content)
else:
    print("Could not find the target return block to replace.")
